      ...
      <div className="two-col">
        ...
        <Card title="Local Patient Explanation (SHAP Waterfall)" badge="Single Prediction" badgeColor="purple">
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#94a3b8', fontWeight: 600, marginBottom: '4px' }}>
                Base Probability
              </div>
              <div style={{ fontSize: '1.35rem', fontWeight: 700, color: '#cbd5e1' }}>
                {(data.waterfall_example?.base_value * 100).toFixed(1)}%
              </div>
            </div>

            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#64748b' }}>
              <span style={{ fontSize: '0.78rem', fontWeight: 600, color: '#94a3b8' }}>SHAP Impact</span>
              <span style={{ fontSize: '1.1rem', color: '#818cf8' }}>➔</span>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                    <span style={{ fontSize: '0.85rem', fontWeight: 600, color: '#f1f5f9', textTransform: 'capitalize' }}>
                      {w.name}
                    </span>
                    <span style={{ fontSize: '0.72rem', color: '#64748b', fontFamily: 'monospace' }}>
                      Value: <span style={{ color: '#94a3b8', fontWeight: 600 }}>{w.val}</span>
                    </span>
                  </div>

                  {/* Column 2: Visual Impact Bar */}
                  <div style={{ width: '100%', background: 'rgba(255, 255, 255, 0.05)', borderRadius: '6px', height: '10px', overflow: 'hidden', display: 'flex', alignItems: 'center' }}>
                    <motion.div
                      style={{
                        height: '100%',
                        width: `${barPct}%`,
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.75rem' }}>
              ...
              <h4 style={{ fontSize: '0.92rem', fontWeight: 700, color: '#f8fafc', margin: 0, letterSpacing: '0.02em' }}>
                Clinical Interpretation
              </h4>
            </div>

            <p style={{ fontSize: '0.84rem', color: '#cbd5e1', lineHeight: '1.6', margin: 0, paddingLeft: '2.5rem' }}>
              This patient has a <strong style={{ color: '#818cf8' }}>{(data.waterfall_example?.predicted_probability * 100).toFixed(1)}%</strong> risk of readmission, which is significantly higher than the base rate of <strong style={{ color: '#94a3b8' }}>{(data.waterfall_example?.base_value * 100).toFixed(1)}%</strong>. 
              The primary driving factors are <strong style={{ color: '#34d399', textTransform: 'capitalize' }}>{waterfallData[0]?.name}</strong> ({waterfallData[0]?.val}) and <strong style={{ color: '#34d399', textTransform: 'capitalize' }}>{waterfallData[1]?.name}</strong> ({waterfallData[1]?.val}), which collectively add <strong style={{ color: '#34d399' }}>+24%</strong> to the baseline risk. 
